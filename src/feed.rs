...
impl Feed {
    pub fn from_url(url: &str) -> Result<Feed, Box<dyn Error>> {
        ...
        client.get(url).header(header::USER_AGENT, "RSSFilter").send()?.read_to_end(&mut content)?;
    ...
    pub fn filter<F>(&mut self, predicate: F) where F: Fn(FeedItem) -> bool {
        match self {
            Feed::RSS(rss_feed) => {
                let new_items: Vec<rss::Item> = rss_feed.items().iter().filter(|item| {
                    if let Some(title) = item.title() {
                        let feed_item = FeedItem { title: title.to_string() };
                        predicate(feed_item)
                    } else {
                        false
                    }
            ...
            Feed::Atom(atom_feed) => {
                let new_entries: Vec<atom_syndication::Entry> = atom_feed.entries().iter().filter(|entry| {
                    let feed_item = FeedItem {title: entry.title().to_string()};
                    predicate(feed_item)
                }).cloned().collect();
